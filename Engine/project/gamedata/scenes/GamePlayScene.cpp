	...
	for (Obj& obj : objects_) {
		...
		if (IsCollision(obj.obb_, structSphere_)) {//Playerとオブジェクトの当たり判定
			if (player_->GetIsHitObj() == false) {
				...
				float overlap = structSphere_.radius + obj.obb_.size.num[0] - distance;
				
				if (overlap > 0.0f) {
					Vector3 correction = Normalize(direction) * (overlap / 2) * 2.0f;
					structSphere_.center = structSphere_.center - correction;

					player_->SetTranslate(structSphere_.center);
				}
