...
void Player::Updete(const ViewProjection viewProjection) {
	...
	if (isMissWire_ == true) {
		missTimer_++;
		worldTransformObject_.translation_ += wireVelocity_;
	}
	if (missTimer_ >= 15) {
		isMissWire_ = false;
		missTimer_ = 0;
...
void Player::Draw(const ViewProjection viewProjection) {
	...
	if (isSetWire_ == true) {
		line_->Draw(worldTransform2_, worldTransformObject_, viewProjection, lineMaterial_);
	}
	if (isMissWire_ == true) {
		line_->Draw(worldTransform2_, worldTransformObject_, viewProjection, lineMaterial_);
	}
...
void Player::DrawUI() {
	...
	sprite_->Draw(spriteTransform2_, SpriteuvTransform_, sphereMaterial_);
}
